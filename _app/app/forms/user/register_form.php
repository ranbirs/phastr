...
class Register_form extends Form
{
	...
	public function fields()
	{
		...
		$this->input('register_email', 'Email', $params = ['attr' => ['class' => 'form-control']]);
		
		$this->validate('register_email', 'email');
		$this->validate('register_email', ['maxlength' => 128]);
		
		$this->input('register_password', 'Password', $params = ['type' => 'password', 'attr' => ['class' => 'form-control']]);
		
		$this->validate('register_password', ['maxlength' => 32]);
		$this->validate('register_password', ['minlength' => 8]);
		
		$this->button('register_submit', 'Submit', $params = ['attr' => ['class' => ['btn', 'btn-primary']]]);

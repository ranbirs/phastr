...
class Simple_form extends \sys\modules\Form
{
	...
	public function fields()
	{
		$this->input('test_custom_validation_field', ' ',
			$params = [
				'attr' => ['placeholder' => 'A number that starts with 123...', 'class' => 'form-control'], 
			]);
		...
		$this->input('test_number_field_optional', ' ',
			$params = [
				'type' => 'number',
				'attr' => ['placeholder' => 'Optional number field...', 'class' => 'form-control']
			]);
		...
		$this->expire(false);
		$this->message('<strong>Good job...</strong><br>The token for this form will not "expire" during the current session (so it may be re-submitted successfully without refreshing the page).', 'success');
	}
